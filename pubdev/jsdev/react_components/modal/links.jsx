    ...
    render() {
        ...
        return (
            <div>
                ...
                <div>
                    { thing || entry ? <h4 style={{float: 'left'}}>Actions</h4> : <div> { this.props.data.length > 0 ? <h4 style={{float: 'left'}}>Select a link for options</h4> : null } </div> }
                    <ButtonGroup style={{float: 'right'}}>
                        {thing || entry ? <Button onClick={this.RemoveLink} >Remove Link</Button> : null }
                    </ButtonGroup>
                </div>
            </div>
        );
