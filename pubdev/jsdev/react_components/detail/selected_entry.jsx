...
let FileUpload              = require( '../components/file_upload.jsx' );
let DeleteEntry             = require( '../modal/delete.jsx' ).DeleteEntry;
let Summary                 = require( '../components/summary.jsx' );
let Task                    = require( '../components/task.jsx' );
...
let IncidentTable           = require( '../components/incident_table.jsx' );
let SignatureTable          = require( '../components/signature_table.jsx' );
...
let CustomMetaDataTable     = require( '../components/custom_metadata_table.jsx' );

let SelectedEntry = React.createClass( {
...
let EntryIterator = React.createClass( {
    render: function() {
        let rows = [];
        ...
        if ( data[0] == undefined ) {
            if ( type != 'alertgroup' ) {
                return (
                    <div>
                        <div style={{color:'blue'}}>No entries found. Click the green Add Entry button if you would like to create one</div>
                    </div>
                );
            } else {
                return (
...
let AlertParent = React.createClass( {
    getInitialState: function() {
        let arr = [];
        return {
            activeIndex: arr,
...
let AlertHeader = React.createClass( {
    render: function() {
        return (
...
let AlertBody = React.createClass( {
    getInitialState: function() {
        return {
...
let AlertRow = React.createClass( {
    render: function() {
        ...
        if ( Array.isArray( this.props.dataFlair[value] ) ) {
            for ( let i = 0; i < this.props.dataFlair[value].length; i++ ) {
                arr.push( <div dangerouslySetInnerHTML={{ __html: $( '<div>' ).text( this.props.dataFlair[value][i] ).html() }}/> );
                arr.push( <br/> );
            }
        } else {
            arr.push( <div dangerouslySetInnerHTML={{ __html: this.props.dataFlair[value] }}/> );
        }
        return (
...
let AlertRowBlank = React.createClass( {
    render: function() {
...
let EntryParent = React.createClass( {
    getInitialState: function() {
        return {
...
let EntryData = React.createClass( { 
    getInitialState: function() {
