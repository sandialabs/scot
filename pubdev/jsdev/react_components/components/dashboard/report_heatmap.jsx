...
class ReportHeatmap extends PureComponent {
    ...
    loadHeatMap() {
        ...
        let url = '/scot/api/v2/graph/dhheatmap?collection='+this.state.collection+'&type='+this.state.type+'&year='+this.state.year; 
        d3.json(
            url,
            function( error, data ) {
                /*
				...
				*/
                ...
                // Legend
                let legend = graph.selectAll( '.legend' )
    ...
    exportToPNG() {
        let svgString = new XMLSerializer().serializeToString( document.querySelector( '#report_heatmap' ) );
