...
class ReportArt extends PureComponent {
	...
	updateChart() {
		...
		// Bars
		let dates = this.svg.selectAll( '.date' )
		...
		this.svg.selectAll( '.date' ).selectAll( '.bar' )
			.transition()
				.attr( 'width', barScale.bandwidth() )
				.attr( 'x', d => barScale( d.name ) )
		...
		let AvgHolder = this.svg.append( 'g' )
        ...
        return (
            <div className="dashboard">
                ...
                <label htmlFor="length" style={{display: "inline-block", textAlign:"right"}}>Length =&nbsp;
                    <input className='report_name' type="number" min="1" step="1" value={this.state.length} id="length" onChange={this.lengthChange} disabled={formDisabled} />
                </label>
