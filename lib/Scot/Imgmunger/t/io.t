...
require_ok('Scot::Imgmunger::Io');
my $io = Scot::Imgmunger::Io->new({env => $env});
...
my $io_entry = $io->retrieve_item('entry',$entry->id);

my $got = $io_entry->as_hash;
my $expect = $entry->as_hash;
my $expect_update_time = $expect->{updated};

cmp_deeply($got, $expect, "Retrieve Item works");

sleep 1; # ensure updated time will change
my $newhtml = "<html><body><p>foobar strikes again</p></body></html>";
$io->update_item($entry, $newhtml);

$io_entry = $io->retrieve_item('entry', $entry->id);

$got = $io_entry->as_hash;
$expect = $entry->as_hash;

is ($got->{body_flair}, $newhtml, "updated body_flair");
ok ($got->{updated} > $expect_update_time, "update time updated");

my $testname = "foobar.exe";
my $expectbase = "foobar";
my $expectext = "exe";

my ($gotbase, $gotext) = $io->get_base_ext($testname);

is ($gotbase, $expectbase, "get_base_ext got correct base");
is ($gotext, $expectext, "get_base_ext got correct extension");

my $test_store_dir  = "/tmp";
my $test_entry_id   = 4444;
my $newname         = "boombaz.png";
my $expectfqn       = "/tmp/entry/4444/boombaz.png";
my $gotfqn  = $io->create_fqn($test_store_dir, $test_entry_id, $newname);
is ($gotfqn, $expectfqn, "create_fqn works");

my $data_uri_link = <<'EOF';
...
my ($gotmime, $gotenc, $gotdata) = $io->parse_uri($data_uri_link);
is ($gotmime, "image/png", "got mimetype from data uri");
is ($gotenc, "base64", "got encoding from data uri");

my ($gfqn, $gnn, $gn) = ($io->save_data_uri($data_uri_link, 44));
ok (-e $gfqn, "data uri link was cached into file");
