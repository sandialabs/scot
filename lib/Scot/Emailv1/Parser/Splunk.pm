...
sub is_non_html_splunk_alert {
    my $self    = shift;
    my $body    = shift;
    
    if ( $body =~ /NON-HTML-SPLUNK/ ) {
        return 1;
    }
    return undef;
}
...
sub parse_message {
    ...
    if ( $self->is_non_html_splunk_alert($body)) {
        $log->warn("NON HTML Messaged Detected!");
        my %json = (
            subject => "WARN (non-html alert): ". $message->{subject},
            message_id  => $message->{message_id},
            body_plain  => $message->{body_plain},
            body        => $body,
            data        => [ { email => $body, columns => ['email'] } ],
            source      => [ 'email', 'splunk' ],
            tag         => [ 'non-html-alert' ],
            columns     => ['email'],
        );
...
sub normalize_body {
    ...
    unless ( $body =~ /\<html.*\>/ or $body =~ /DOCTYPE html/ ) {
        my $warning = qq{ NON-HTML-SPLUNK: Splunk Message was not in HTML format!  Parsing will be incomplete.  Please set Splunk alert to HTML output via Splunk."};
        $log->warn($warning);
...
sub extract_rows_columns {
    ...
    if (scalar(@columns) < 1) {
        $self->env->log->error("COLUMNS NOT FOUND: ",
                               { filter =>\&Dumper, value => $report });
    }
