...
export default class LinkWarning extends React.Component {
  ...
  componentWillMount = () => {
    ...
    if (linkRegResult !== undefined || linkRegResult !== null && myDomainRegResult !== undefined || myDomainRegResult !== undefined) {
      let linkDomain = linkRegResult[3];
      let myDomain = myDomainRegResult[3];
      if (linkDomain === myDomain) {
        this.proceed();
      }
    } else if (linkRegResult === undefined || linkRegResult === null) {
      this.proceed();
    }
    /*
