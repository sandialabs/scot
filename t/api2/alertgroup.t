...
my $alertgroup_id   = $t->tx->res->json->[0]->{alertgroup};
...
$t->get_ok("/scot/api/v3/alertgroup" => {},
    ...
    ->json_is('/records/0/message_id'   => '112233445566778899aabbccddeeff')
    ->json_is('/records/0/subject'      => 'test message 1')
    ->json_is('/records/0/columns/0'    => 'foo')
    ->json_is('/records/0/columns/1'    => 'bar')
    ->json_is('/records/0/columns/2'    => 'data');
...
$t->get_ok("/scot/api/v3/alertgroup/$alertgroup_id/alert" => {},
...
$t->get_ok("/scot/api/v3/alertgroup/$alertgroup_id/entity" => {},
...
$t->put_ok("/scot/api/v3/alert/$alert1_id" => json => 
    {data => $alert1_data,
     columns => [ qw(foo bar boom) ]}
);
$t->put_ok("/scot/api/v3/alert/$alert2_id" => json => 
...
$t->get_ok("/scot/api/v3/alertgroup/$alertgroup_id/tag" => {},
...
$t->get_ok("/scot/api/v3/alertgroup/$alertgroup_id/history" => {},
    "Getting alertgroup history")
    ->status_is(200);

$t  ->post_ok('/scot/api/v3/entry'  => json => {
...
$t->get_ok("/scot/api/v3/alert/$alert1_id/entry")
    ->status_is(200)
...
$t->put_ok("/scot/api/v3/alertgroup/$alertgroup_id" => json =>
    { status => 'closed' } 
)->status_is(200)
 ->json_is("/status" => "ok");
...
foreach my $ag (@ags) {
    $t->post_ok(
        '/scot/api/v3/alertgroup'   => json => $ag
    )->status_is(200);
}
...
$t->get_ok("/scot/api/v3/alertgroup/7")
    ->status_is(200);
$t->get_ok("/scot/api/v3/alertgroup/7")
    ->status_is(200);
$t->get_ok("/scot/api/v3/alertgroup/7")
    ->status_is(200);
$t->get_ok("/scot/api/v3/alertgroup/6")
    ->status_is(200);
$t->get_ok("/scot/api/v3/alertgroup/6")
    ->status_is(200);
$t->get_ok("/scot/api/v3/alertgroup/5")
    ->status_is(200);
...
$t->get_ok("/scot/api/v3/alertgroup/7/alert")
    ->status_is(200);
